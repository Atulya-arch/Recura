...
import { AuditEvent } from '../../../shared/types.js';
import { CheckCircle2, AlertTriangle, ShieldCheck, Clock, RefreshCw, XCircle, ArrowRight } from 'lucide-react';
import { FormatMoney } from './FormatMoney';

export const Timeline: React.FC<{ events: AuditEvent[]; amountMinor: number }> = ({ events, amountMinor }) => {
      ...
      {sorted.map((evt) => {
        ...
        switch (evt.eventType) {
          ...
          case 'AI_RECOMMENDATION':
            icon = <ArrowRight className="w-3.5 h-3.5 text-purple-600" />;
        ...
        return (
          <div key={evt.id} className="relative flex items-start space-x-3 text-sm">
            <div className={`absolute -left-6 top-0.5 flex items-center justify-center w-5 h-5 rounded-full border ${dotBg} shrink-0`}>
              {icon}
            </div>
            <div className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-3.5 shadow-sm">
              <div className="flex items-center justify-between gap-2">
                <span className={`font-extrabold text-xs ${titleColor}`}>{title}</span>
                <span className="text-[10px] text-slate-400 font-mono shrink-0">{timeStr}</span>
              </div>
              {evt.eventType === 'PAYMENT_RECOVERED' && (
                <div className="mt-1 text-slate-950 font-black text-xs">
                  Recovered: <FormatMoney amountMinor={amountMinor} />
                </div>
              )}
              {evt.metadata?.diagnosis && (
                <p className="mt-1 text-[11px] text-slate-600 font-medium">{evt.metadata.diagnosis}</p>
              )}
              {evt.metadata?.rationale && (
                <p className="mt-1 text-[11px] text-slate-500 italic">"{evt.metadata.rationale}"</p>
              )}
            </div>
          </div>
        );
