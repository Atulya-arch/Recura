...
import { RecoveryStatus, PaymentStatus } from '../../shared/enums.js';
...
router.get('/dashboard/metrics', async (req, res) => {
    try {
        ...
        for (const t of failedTxs)
            revenueAtRiskMinor += t.amountMinor;
        let recoveredRevenueMinor = 0;
        for (const c of allCases)
            recoveredRevenueMinor += c.recoveredAmountMinor;
...
router.post('/recovery-cases/:id/hinglish-negotiate', async (req, res) => {
    try {
        ...
        const extraction = await aiService.extractPromiseToPay(customerReply, cust ? cust.name : 'Customer', tx ? tx.orderId : 'ORD', amountFormatted);
