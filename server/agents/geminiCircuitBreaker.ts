...
export class GeminiCircuitBreaker {
   ...
   */
  public trip(cooldownSeconds = 60): void {
    this.state = 'OPEN';
  ...
  ): Promise<{ result: T; fromFallback: boolean; reason?: string }> {
    this.totalRequests++;
  ...
  private extractRetryDelay(err: any): number {
    const text = (err?.message || '') + ' ' + JSON.stringify(err || {});
    const match =
      text.match(/retry(?:Delay| in)?[:\s"]+(\d+(?:\.\d+)?)\s*s/i) ||
  ...
  private classifyError(err: any): string {
    ...
    if (msg.includes('404') || msg.includes('not found')) return 'MODEL_NOT_FOUND_404';
    if (msg.includes('fetch failed') || msg.includes('econnrefused') || msg.includes('network')) return 'NETWORK_ERROR';
    if (msg.includes('unauthorized') || msg.includes('api_key') || msg.includes('401') || msg.includes('403')) return 'AUTH_ERROR';
