import { Policy, RecoveryCase, Transaction, Customer, RecoveryAction } from '../../shared/types.js';
...
export class PolicyEngine {
  public static DEFAULT_POLICY: Omit<Policy, 'id' | 'merchantId' | 'updatedAt'> = {
    maxRetries: 3,
  ...
  static isEligible(
    ...
    customer: Customer,
    existingCase?: RecoveryCase | null,
   ...
   */
  static evaluateAiRecommendation(
    recommendation: { recommendedAction: RecoveryActionType; confidence: number },
    ...
    if (recommendation.recommendedAction === RecoveryActionType.IMMEDIATE_RETRY || recommendation.recommendedAction === RecoveryActionType.DELAYED_RETRY) {
      const retriesDone = previousActions.filter(
        a => (a.actionType === RecoveryActionType.IMMEDIATE_RETRY || a.actionType === RecoveryActionType.DELAYED_RETRY) && a.status === 'EXECUTED'
      ).length;
