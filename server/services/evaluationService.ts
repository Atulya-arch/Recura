...
export class EvaluationService {
   ...
   */
  static async runBatchEvaluation(): Promise<EvaluationResults> {
    ...
    for (const txId of eligibleTxIds) {
      ...
      if (recCase) {
        ...
        if (recCase.status === 'STOPPED') stoppedCount++;
        if (recCase.status === 'ESCALATED') escalationsCount++;
        if (recCase.status === 'FAILED') failedCount++;
      }
    }
