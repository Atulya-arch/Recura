...
export class SimulationPaymentProvider {
    name = 'SIMULATION';
    executedKeys = new Set();
    simulatedTxStates = new Map();
    setScenarioForTx(txId, scenario, verifiedStatus = PaymentStatus.SUCCESS) {
    ...
    async retryPayment(request) {
        ...
        const scenario = configured?.scenario || this.inferScenarioFromTxId(request.transactionId, request.attemptNumber);
        if (scenario === 'TIMEOUT') {
            return {
                success: false,
    ...
    inferScenarioFromTxId(txId, attempt) {
        if (txId.includes('timeout'))
            return 'TIMEOUT';
        if (txId.includes('perm'))
            return 'PERMANENT_FAILURE';
        if (txId.includes('temp'))
